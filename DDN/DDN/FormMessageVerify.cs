...
namespace DDN
{
    public partial class FormMessageVerify : Form
    {
        ...
        void reFresh(object state) {
            this.labelMsgAmount.Text = "消息验证: " + Manager.Instance.msgMgr.mList.Count+"条";
            this.flowLayoutPanel.Controls.Clear();

            for (int i = Manager.Instance.msgMgr.mList.Count-1; i >= 0 ; i--)
            {
                MsgModel model = Manager.Instance.msgMgr.mList[i];
